...
class AFMChainTensileTestCurveFitCharacterizer(
    ...
    def set_user_parameters(self):
        ...
        f_c_dot_label_list = [
            r'$\textrm{c}u\textrm{FJC scission},~\dot{f}_c='+'10^{0:d}'.format(f_c_dot_exponent_list[i])+'~nN/sec$'
            for i in range(len(f_c_dot_list))
        ]
        ...
        if self.comm_rank == 0:
            ...
            r_nu, f_c = (
                chain_data_file_load_func(
                    data_file_prefix, cp.chain_data_directory)
            )
            ...
            kappa_nu, kappa_nu_intgr_nu) = (
                l_nu_eq_nu_kappa_nu_fit_func(r_nu, f_c)
            )
            ...
            save_pickle_object(self.savedir, r_nu, data_file_prefix+"r_nu")
            save_pickle_object(
                self.savedir, r_nu_fit, data_file_prefix+"r_nu_fit")
            save_pickle_object(
                self.savedir, r_nu_fit_intgr_nu,
                data_file_prefix+"r_nu_fit_intgr_nu")
            
            save_pickle_object(
                self.savedir, lmbda_c_eq, data_file_prefix+"lmbda_c_eq")
            save_pickle_object(
                self.savedir, lmbda_c_eq_fit, data_file_prefix+"lmbda_c_eq_fit")
            save_pickle_object(
                self.savedir,
                lmbda_c_eq_intgr_nu, data_file_prefix+"lmbda_c_eq_intgr_nu")
            save_pickle_object(
                self.savedir, lmbda_c_eq_fit_intgr_nu,
                data_file_prefix+"lmbda_c_eq_fit_intgr_nu")
            
            save_pickle_object(self.savedir, f_c, data_file_prefix+"f_c")
            save_pickle_object(
                self.savedir, f_c_fit, data_file_prefix+"f_c_fit")
            save_pickle_object(
                self.savedir, f_c_fit_intgr_nu,
                data_file_prefix+"f_c_fit_intgr_nu")
            
            save_pickle_object(self.savedir, xi_c, data_file_prefix+"xi_c")
            save_pickle_object(
                self.savedir, xi_c_fit, data_file_prefix+"xi_c_fit")
            save_pickle_object(
                self.savedir, xi_c_intgr_nu, data_file_prefix+"xi_c_intgr_nu")
            save_pickle_object(
                self.savedir, xi_c_fit_intgr_nu,
                data_file_prefix+"xi_c_fit_intgr_nu")
            
            save_pickle_object(
                self.savedir, xi_c_max, data_file_prefix+"xi_c_max")

            # print and save numerical results in text files
            print("\nl_b_eq = {}".format(l_b_eq))
            print("l_nu_eq = {}".format(l_nu_eq))
            print("l_nu_eq_intgr_nu = {}".format(l_nu_eq_intgr_nu))
            print("nu_b = {}".format(nu_b))
            print("nu_b_intgr_nu = {}".format(nu_b_intgr_nu))
            print("nu = {}".format(nu))
            print("intgr_nu = {}".format(intgr_nu))
            print("zeta_b_char = {}".format(zeta_b_char))
            print("zeta_nu_char = {}".format(zeta_nu_char))
            print("zeta_nu_char_intgr_nu = {}".format(zeta_nu_char_intgr_nu))
            print("kappa_b = {}".format(kappa_b))
            print("kappa_b_intgr_nu = {}".format(kappa_b_intgr_nu))
            print("kappa_nu = {}".format(kappa_nu))
            print("kappa_nu_intgr_nu = {}".format(kappa_nu_intgr_nu))
            print("\n")

            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-l_b_eq'+'.txt',
                [l_b_eq])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-l_nu_eq'+'.txt',
                [l_nu_eq])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-l_nu_eq_intgr_nu'+'.txt',
                [l_nu_eq_intgr_nu])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-nu_b'+'.txt',
                [nu_b])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-nu_b_intgr_nu'+'.txt',
                [nu_b_intgr_nu])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-nu'+'.txt',
                [nu])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-intgr_nu'+'.txt',
                [intgr_nu])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-zeta_b_char'+'.txt',
                [zeta_b_char])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-zeta_nu_char'+'.txt',
                [zeta_nu_char])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-zeta_nu_char_intgr_nu'+'.txt',
                [zeta_nu_char_intgr_nu])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-kappa_b'+'.txt',
                [kappa_b])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-kappa_b_intgr_nu'+'.txt',
                [kappa_b_intgr_nu])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-kappa_nu'+'.txt',
                [kappa_nu])
            np.savetxt(
                self.savedir+data_file_prefix+'-composite-uFJC-curve-fit-kappa_nu_intgr_nu'+'.txt',
                [kappa_nu_intgr_nu])
            ...
            for f_c_dot_indx in range(len(cp.f_c_dot_list)):
                ...
                for f_c_indx in range(cp.f_c_num_steps):
                    ...
                    lmbda_c_eq_val = (
                        rate_dependent_single_chain.lmbda_c_eq_func(
                            lmbda_nu_val)
                    )
                    p_nu_sci_hat_val = (
                    ...
                    if f_c_indx == 0:
                        ...
                        p_nu_sci_hat_cmltv_intgrl_val = (
                            rate_dependent_single_chain.p_nu_sci_hat_cmltv_intgrl_func(
                                p_nu_sci_hat_val, t_val,
                                p_nu_sci_hat[f_c_indx-1], t_steps[f_c_indx-1],
                        ...
                        overline_epsilon_cnu_diss_hat_val = (
                            epsilon_cnu_diss_hat_val
                            / rate_dependent_single_chain.zeta_nu_char
                        )
                    ...
                    p_nu_sci_hat_cmltv_intgrl.append(
                            p_nu_sci_hat_cmltv_intgrl_val)
                    gamma_c.append(gamma_c_val)
                    epsilon_cnu_diss_hat.append(epsilon_cnu_diss_hat_val)
                    overline_epsilon_cnu_diss_hat.append(
                        overline_epsilon_cnu_diss_hat_val)
                
                t_steps___f_c_dot_chunk.append(t_steps)
                xi_c___f_c_dot_chunk.append(xi_c)
                lmbda_nu___f_c_dot_chunk.append(lmbda_nu)
                lmbda_c_eq___f_c_dot_chunk.append(lmbda_c_eq)
                gamma_c___f_c_dot_chunk.append(gamma_c)
                overline_epsilon_cnu_diss_hat___f_c_dot_chunk.append(
                    overline_epsilon_cnu_diss_hat)
            
            save_pickle_object(
                self.savedir, t_steps___f_c_dot_chunk,
                data_file_prefix+"rate_dependent_t_steps___f_c_dot_chunk")
            save_pickle_object(
                self.savedir, xi_c___f_c_dot_chunk,
                data_file_prefix+"rate_dependent_xi_c___f_c_dot_chunk")
            save_pickle_object(
                self.savedir, lmbda_nu___f_c_dot_chunk,
                data_file_prefix+"rate_dependent_lmbda_nu___f_c_dot_chunk")
            save_pickle_object(
                self.savedir, lmbda_c_eq___f_c_dot_chunk,
                data_file_prefix+"rate_dependent_lmbda_c_eq___f_c_dot_chunk")
            save_pickle_object(
                self.savedir, gamma_c___f_c_dot_chunk,
                data_file_prefix+"rate_dependent_gamma_c___f_c_dot_chunk")
            save_pickle_object(
                self.savedir, overline_epsilon_cnu_diss_hat___f_c_dot_chunk,
                data_file_prefix+"rate_dependent_overline_epsilon_cnu_diss_hat___f_c_dot_chunk")
            ...
            for f_c_indx in range(cp.f_c_num_steps):
                ...
                lmbda_nu_val = (
                    rate_independent_single_chain.lmbda_nu_xi_c_hat_func(
                        xi_c_val)
                )
                ...
                overline_epsilon_cnu_sci_hat_val = (
                    epsilon_cnu_sci_hat_val
                    / rate_independent_single_chain.zeta_nu_char
                )
                ...
                overline_epsilon_cnu_sci_hat.append(
                    overline_epsilon_cnu_sci_hat_val)
                epsilon_cnu_diss_hat.append(epsilon_cnu_diss_hat_val)
                overline_epsilon_cnu_diss_hat.append(
                    overline_epsilon_cnu_diss_hat_val)
            ...
            save_pickle_object(
                self.savedir, xi_c,
                data_file_prefix+"rate_independent_xi_c")
            save_pickle_object(
                self.savedir, lmbda_nu,
                data_file_prefix+"rate_independent_lmbda_nu")
            save_pickle_object(
                self.savedir, lmbda_c_eq,
                data_file_prefix+"rate_independent_lmbda_c_eq")
            save_pickle_object(
                self.savedir, p_c_sci_hat,
                data_file_prefix+"rate_independent_p_c_sci_hat")
            save_pickle_object(
                self.savedir, overline_epsilon_cnu_sci_hat,
                data_file_prefix+"rate_independent_overline_epsilon_cnu_sci_hat")
            save_pickle_object(
                self.savedir, overline_epsilon_cnu_diss_hat,
                data_file_prefix+"rate_independent_overline_epsilon_cnu_diss_hat")
    ...
    def finalization(self):
        ...
        if self.comm_rank == 0:
            ...
            r_nu = load_pickle_object(self.savedir, data_file_prefix+"r_nu")
            r_nu_fit = load_pickle_object(
                self.savedir, data_file_prefix+"r_nu_fit")
            r_nu_fit_intgr_nu = load_pickle_object(
                self.savedir, data_file_prefix+"r_nu_fit_intgr_nu")
            
            lmbda_c_eq = load_pickle_object(
                self.savedir, data_file_prefix+"lmbda_c_eq")
            lmbda_c_eq_fit = load_pickle_object(
                self.savedir, data_file_prefix+"lmbda_c_eq_fit")
            lmbda_c_eq_intgr_nu = load_pickle_object(
                self.savedir, data_file_prefix+"lmbda_c_eq_intgr_nu")
            lmbda_c_eq_fit_intgr_nu = load_pickle_object(
                self.savedir, data_file_prefix+"lmbda_c_eq_fit_intgr_nu")
            
            f_c = load_pickle_object(self.savedir, data_file_prefix+"f_c")
            f_c_fit = load_pickle_object(
                self.savedir, data_file_prefix+"f_c_fit")
            f_c_fit_intgr_nu = load_pickle_object(
                self.savedir, data_file_prefix+"f_c_fit_intgr_nu")
            
            xi_c = load_pickle_object(self.savedir, data_file_prefix+"xi_c")
            xi_c_fit = load_pickle_object(
                self.savedir, data_file_prefix+"xi_c_fit")
            xi_c_intgr_nu = load_pickle_object(
                self.savedir, data_file_prefix+"xi_c_intgr_nu")
            xi_c_fit_intgr_nu = load_pickle_object(
                self.savedir, data_file_prefix+"xi_c_fit_intgr_nu")
            
            xi_c_max = load_pickle_object(
                self.savedir, data_file_prefix+"xi_c_max")

            rate_dependent_t_steps___f_c_dot_chunk = load_pickle_object(
                self.savedir,
                data_file_prefix+"rate_dependent_t_steps___f_c_dot_chunk")
            rate_dependent_xi_c___f_c_dot_chunk = load_pickle_object(
                self.savedir,
                data_file_prefix+"rate_dependent_xi_c___f_c_dot_chunk")
            rate_dependent_lmbda_nu___f_c_dot_chunk = load_pickle_object(
                self.savedir,
                data_file_prefix+"rate_dependent_lmbda_nu___f_c_dot_chunk")
            rate_dependent_lmbda_c_eq___f_c_dot_chunk = load_pickle_object(
                self.savedir,
                data_file_prefix+"rate_dependent_lmbda_c_eq___f_c_dot_chunk")
            rate_dependent_gamma_c___f_c_dot_chunk = load_pickle_object(
                self.savedir,
                data_file_prefix+"rate_dependent_gamma_c___f_c_dot_chunk")
            rate_dependent_overline_epsilon_cnu_diss_hat___f_c_dot_chunk = (
                load_pickle_object(
                    self.savedir,
                    data_file_prefix+"rate_dependent_overline_epsilon_cnu_diss_hat___f_c_dot_chunk")
            )
            
            rate_independent_xi_c = load_pickle_object(
                self.savedir, data_file_prefix+"rate_independent_xi_c")
            rate_independent_lmbda_nu = load_pickle_object(
                self.savedir, data_file_prefix+"rate_independent_lmbda_nu")
            rate_independent_lmbda_c_eq = load_pickle_object(
                self.savedir, data_file_prefix+"rate_independent_lmbda_c_eq")
            rate_independent_p_c_sci_hat = load_pickle_object(
                self.savedir, data_file_prefix+"rate_independent_p_c_sci_hat")
            rate_independent_overline_epsilon_cnu_diss_hat = load_pickle_object(
                self.savedir,
                data_file_prefix+"rate_independent_overline_epsilon_cnu_diss_hat")
            rate_independent_overline_epsilon_cnu_sci_hat = load_pickle_object(
                self.savedir,
                data_file_prefix+"rate_independent_overline_epsilon_cnu_sci_hat")
            ...
            fig = plt.figure()
            plt.scatter(
                r_nu, f_c, color='blue', marker='o', alpha=1, linewidth=2.5,
                label=cp.paper_authors2polymer_type_label_dict[self.paper_authors])
            plt.plot(
                r_nu_fit, f_c_fit, linestyle='--', color='red', alpha=1,
                linewidth=2.5, label=r'$\textrm{c}u\textrm{FJC model fit}$')
            ...
            save_current_figure(
                self.savedir, r'$r_{\nu}~(nm)$', 30, r'$f_c~(nN)$', 30,
                data_file_prefix+"-f_c-vs-r_nu-cuFJC-curve-fit")
            ...
            plt.scatter(
                ...
                label=cp.paper_authors2polymer_type_label_dict[self.paper_authors])
            plt.plot(
                r_nu_fit_intgr_nu, f_c_fit_intgr_nu, linestyle='--',
                color='red', alpha=1, linewidth=2.5,
                label=r'$\textrm{c}u\textrm{FJC model fit}$')
            plt.legend(loc='best', fontsize=18)
            plt.grid(True, alpha=0.25)
            plt.xlim(
                cp.paper_authors_chain2xlim_chain_mechanical_response_plot[self.paper_authors][self.chain])
            ...
            save_current_figure(
                self.savedir, r'$r_{\nu}~(nm)$', 30, r'$f_c~(nN)$', 30,
                data_file_prefix+"-intgr_nu-f_c-vs-r_nu-cuFJC-curve-fit")
            
            fig = plt.figure()
            plt.scatter(
                lmbda_c_eq, xi_c, color='blue', marker='o', alpha=1,
                linewidth=2.5,
                label=cp.paper_authors2polymer_type_label_dict[self.paper_authors])
            plt.plot(
                lmbda_c_eq_fit, xi_c_fit, linestyle='--', color='red',
            ...
            save_current_figure(
                ...
                data_file_prefix+"-xi_c-vs-lmbda_c_eq-cuFJC-curve-fit")
            
            fig = plt.figure()
            plt.scatter(
                lmbda_c_eq_intgr_nu, xi_c_intgr_nu, color='blue', marker='o',
                alpha=1, linewidth=2.5,
                label=cp.paper_authors2polymer_type_label_dict[self.paper_authors])
            plt.plot(
                lmbda_c_eq_fit_intgr_nu, xi_c_fit_intgr_nu,
            ...
            save_current_figure(
                self.savedir, r'$\lambda_c^{eq}$', 30, r'$\xi_{c}$', 30,
                data_file_prefix+"-intgr_nu-xi_c-vs-lmbda_c_eq-cuFJC-curve-fit")
            ...
            for f_c_dot_indx in range(len(cp.f_c_dot_list)):
                t_max = max(
                    [
                        t_max,
                        rate_dependent_t_steps___f_c_dot_chunk[f_c_dot_indx][-1]
                    ])
                ax1.semilogx(
                    rate_dependent_t_steps___f_c_dot_chunk[f_c_dot_indx],
                    rate_dependent_xi_c___f_c_dot_chunk[f_c_dot_indx],
                    linestyle='-', color=cp.f_c_dot_color_list[f_c_dot_indx],
                    alpha=1, linewidth=2.5,
                    label=cp.f_c_dot_label_list[f_c_dot_indx])
                ax2.semilogx(
                    rate_dependent_t_steps___f_c_dot_chunk[f_c_dot_indx],
                    rate_dependent_lmbda_nu___f_c_dot_chunk[f_c_dot_indx],
                    linestyle='-', color=cp.f_c_dot_color_list[f_c_dot_indx],
                    alpha=1, linewidth=2.5)
                ax3.semilogx(
                    rate_dependent_t_steps___f_c_dot_chunk[f_c_dot_indx],
                    rate_dependent_gamma_c___f_c_dot_chunk[f_c_dot_indx],
                    linestyle='-', color=cp.f_c_dot_color_list[f_c_dot_indx],
                    alpha=1, linewidth=2.5)
                ax4.semilogx(
                    rate_dependent_t_steps___f_c_dot_chunk[f_c_dot_indx],
                    rate_dependent_overline_epsilon_cnu_diss_hat___f_c_dot_chunk[f_c_dot_indx],
                    linestyle='-', color=cp.f_c_dot_color_list[f_c_dot_indx],
                    alpha=1, linewidth=2.5)
            ax1.hlines(
                y=xi_c_max, xmin=0, xmax=t_max, linestyle='--', color='black',
                alpha=1, linewidth=1)
            ...
            save_current_figure_no_labels(
                self.savedir,
                data_file_prefix+"-rate-dependent-xi_c-lmbda_nu-gamma_c-overline_epsilon_cnu_diss_hat-vs-time")
            ...
            fig, (ax1, ax2, ax3, ax4) = plt.subplots(
                ...
                sharex=True)
            ax1.scatter(
                lmbda_c_eq_intgr_nu, xi_c_intgr_nu, color='red',
            ...
            for f_c_dot_indx in range(len(cp.f_c_dot_list)):
                lmbda_c_eq_max = max(
                    [
                        lmbda_c_eq_max,
                        rate_dependent_lmbda_c_eq___f_c_dot_chunk[f_c_dot_indx][-1]
                    ])
                ax1.plot(
                    rate_dependent_lmbda_c_eq___f_c_dot_chunk[f_c_dot_indx],
                    rate_dependent_xi_c___f_c_dot_chunk[f_c_dot_indx],
                    linestyle='-', color=cp.f_c_dot_color_list[f_c_dot_indx],
                    alpha=1, linewidth=2.5)
                ax2.plot(
                ...
                ax4.plot(
                    ...
                    alpha=1, linewidth=2.5)
            ax1.hlines(
                y=xi_c_max, xmin=-0.05, xmax=lmbda_c_eq_max + 0.05,
                linestyle='--', color='black', alpha=1, linewidth=1)
            # plot rate-independent chain results
            ax1.plot(
                rate_independent_lmbda_c_eq, rate_independent_xi_c,
                linestyle='-', color='blue', alpha=1, linewidth=2.5,
                label=r'$\textrm{rate-independent c}u\textrm{FJC scission}$')
            ax2.plot(
                rate_independent_lmbda_c_eq, rate_independent_lmbda_nu,
                linestyle='-', color='blue', alpha=1, linewidth=2.5)
            ax3.plot(
                rate_independent_lmbda_c_eq, rate_independent_p_c_sci_hat,
                linestyle='-', color='blue', alpha=1, linewidth=2.5)
            ax4.plot(
                rate_independent_lmbda_c_eq,
                rate_independent_overline_epsilon_cnu_sci_hat, linestyle='-',
                color='black', alpha=1, linewidth=2.5)
            ax4.plot(
                rate_independent_lmbda_c_eq,
                rate_independent_overline_epsilon_cnu_diss_hat, linestyle='-',
            ...
            save_current_figure_no_labels(
                self.savedir,
                data_file_prefix+"-rate-independent-and-rate-dependent-chains-vs-lmbda_c_eq")
            ...
            save_current_figure_no_labels(
                self.savedir,
                data_file_prefix+"-rate-independent-and-rate-dependent-chains-vs-lmbda_c_eq-no-epsilon_cnu_sci")

            # plot chain mechanical response and dissipated chain energy for
            # rate-independent and rate-dependent chain results
            lmbda_c_eq_max = 0
            ...
            for f_c_dot_indx in range(len(cp.f_c_dot_list)):
                ...
                ax2.plot(
                    rate_dependent_lmbda_c_eq___f_c_dot_chunk[f_c_dot_indx][-1],
                    rate_dependent_overline_epsilon_cnu_diss_hat___f_c_dot_chunk[f_c_dot_indx][-1],
                    marker='x', color=cp.f_c_dot_color_list[f_c_dot_indx],
                    alpha=1, linewidth=2.5)
            # ax1.hlines(
            #     y=xi_c_max, xmin=-0.05, xmax=lmbda_c_eq_max + 0.05,
            #     linestyle='--', color='black', alpha=1, linewidth=1)
            ...
            ax2.plot(
                ...
                color='blue', alpha=1, linewidth=2.5)
            ax2.plot(
                rate_independent_lmbda_c_eq[-1],
                rate_independent_overline_epsilon_cnu_diss_hat[-1], marker='x',
                color='blue', alpha=1, linewidth=2.5)
            ax1.legend(loc='best', fontsize=14)
            ax1.tick_params(axis='y', labelsize=20)
            ax1.set_ylabel(r'$\xi_c$', fontsize=24)
            ax1.grid(True, alpha=0.25)
            ...
            save_current_figure_no_labels(
                self.savedir,
                data_file_prefix+"-rate-independent-and-rate-dependent-chains-mech-diss-vs-lmbda_c_eq")

            # plot chain mechanical response, probability of chain scission,
            # and dissipated chain energy for rate-independent and
            # rate-dependent chain results
            lmbda_c_eq_max = 0
            fig, (ax1, ax2, ax3) = plt.subplots(
                3, 1, gridspec_kw={'height_ratios': [1, 1, 1]},
                sharex=True)
            ax1.scatter(
                lmbda_c_eq_intgr_nu, xi_c_intgr_nu, color='red',
                marker='o', alpha=1, linewidth=1,
            ...
            ax2.tick_params(axis='y', labelsize=16)
            ax2.legend(loc='best', fontsize=12)
            ax2.set_ylabel(r'$\gamma_c,~\hat{p}_c^{sci}$', fontsize=24)
            ax2.grid(True, alpha=0.25)
            ...
            save_current_figure_no_labels(
                self.savedir,
                data_file_prefix+"-rate-independent-and-rate-dependent-chains-mech-prob-sci-diss-vs-lmbda_c_eq")
